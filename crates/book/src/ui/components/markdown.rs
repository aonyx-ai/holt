        ...
        <div class="relative">
            <div
                class=move || format!(
                    ...
                    if is_expanded.get() { "max-h-none" } else { "max-h-48 cursor-pointer hover:bg-muted/40" }
                )
                on:click=move |_| if !is_expanded.get() { set_is_expanded.set(true) }
            >
...
fn render_node(node: markdown::mdast::Node) -> impl IntoView {
    match node {
        ...
        markdown::mdast::Node::Code(code) => view! {
            <ExpandableCodeBlock code=code.value.clone() lang=code.lang.clone() />
        }.into_any(),
        markdown::mdast::Node::Definition(_) => view! { <span /> }.into_any(),
        markdown::mdast::Node::Delete(delete) => view! {
            <del>
                {delete.children.iter().map(|child| render_node(child.clone())).collect_view()}
            </del>
        }.into_any(),
        markdown::mdast::Node::Emphasis(emphasis) => view! {
            <em>
                {emphasis.children.iter().map(|child| render_node(child.clone())).collect_view()}
            </em>
        }.into_any(),
        markdown::mdast::Node::Heading(heading) => {
            ...
            view! {
                <div class={format!("{tag} {classes}")}>
                    {heading.children.iter().map(|child| render_node(child.clone())).collect_view()}
                </div>
            }.into_any()
        },
        markdown::mdast::Node::Image(image) => view! {
            <img src={image.url} alt={image.alt} title={image.title} class="max-w-full h-auto rounded" />
        }.into_any(),
        markdown::mdast::Node::ImageReference(_) => view! { <span /> }.into_any(),
        markdown::mdast::Node::InlineCode(code) => view! {
            <code class="bg-muted/50 px-1 py-0.5 rounded text-sm font-mono">{code.value}</code>
        }.into_any(),
        markdown::mdast::Node::InlineMath(math) => view! {
            <span class="math-inline">{math.value}</span>
        }.into_any(),
            ...
            if list.ordered {
                view! {
                    <ol class="list-decimal list-inside space-y-1 ml-4 mb-4">
                        {list.children.iter().map(|child| render_node(child.clone())).collect_view()}
                    </ol>
                }.into_any()
            } else {
                view! {
        ...
        markdown::mdast::Node::Math(math) => view! {
            <div class="math-block">{math.value}</div>
        }.into_any(),
...
pub(crate) fn Markdown(content: String) -> impl IntoView {
    ...
    view! {
        <div class="prose prose-sm max-w-none overflow-hidden break-words">
            {render_node(ast)}
        </div>
    }
}
